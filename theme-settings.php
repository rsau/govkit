 ...
 */
include_once './' . drupal_get_path('theme', 'govkit') . '/includes/common.inc';
 ...
 * @param null $form_id
 */
function govkit_form_system_theme_settings_alter(&$form, $form_state, $form_id = NULL) {
  // @see https://drupal.org/node/943212
  $theme = !empty($form_state['build_info']['args'][0]) ? $form_state['build_info']['args'][0] : FALSE;
  if (isset($form_id) || $theme === FALSE || !in_array('govkit', _govkit_get_base_themes($theme, TRUE))) {
    return;
  }
  ...
  $form['support']['govkit_skip_link_anchor'] = array(
    '#type' => 'textfield',
    '#title' => t('Anchor ID for the “skip link”'),
    '#default_value' => theme_get_setting('govkit_skip_link_anchor'),
    '#field_prefix' => '#',
  ...
  $form['support']['govkit_skip_link_text'] = array(
    '#type' => 'textfield',
    '#title' => t('Text for the “skip link”'),
    '#default_value' => theme_get_setting('govkit_skip_link_text'),
  ...
  $form['development']['govkit_rebuild_registry'] = array(
    '#type' => 'checkbox',
    '#title' => t('Rebuild theme registry on every page.'),
    '#default_value' => theme_get_setting('govkit_rebuild_registry'),
